...
| ------- | ------ | --------------------------- |
| [`patchproto`](../patchproto/) | `proto.Message` | 42 / 42 — it is the reference |
| [`patchstruct`](../patchstruct/) | a hand-written Go struct | 38 / 42 |
| [`patchjson`](../patchjson/) | schema-less JSON | 29 / 42 |
...
| an empty list or map exists in protobuf and is absent in JSON — protojson omits it | 6 |
| a field number cannot be checked without a schema, so it is refused first | 3 |
| there are no declared types to compare for `move`/`copy` | 1 |
